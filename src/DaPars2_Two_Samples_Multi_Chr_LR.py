...
def Each_Thread_3UTR_estimation_list_version_sample_ids(curr_thread_UTR_events_ids, UTR_events_dict, All_sample_coverage_weights, num_samples, Output_result_file, All_samples_coverage_shared_dict, Coverage_threshold):
    ...
    for curr_3UTR_id in curr_thread_UTR_events_ids:
        ...
        select_mean_squared_error, selected_break_point, UTR_abundances = De_Novo_3UTR_Coverage_estimation_Genome_for_multiple_samples(curr_3UTR_all_samples_bp_coverage, region_start, region_end,curr_strand,All_sample_coverage_weights, Coverage_threshold, curr_3UTR_id)
            ...
            if num_non_zero > 0:
                ...
                for i in range(num_samples):
                    if UTR_abundances[0][i] != 'NA':
                        if float(UTR_abundances[0][i]) + float(UTR_abundances[1][i]) == 0:
                        ...
                        line_write.extend(['NA']*1)
                if UTR_abundances[2] != 'NA':
                    line_write.append("%.6e" % UTR_abundances[2])
                else:
                    line_write.extend(['NA']*1)
...
def De_Novo_3UTR_Coverage_estimation_Genome_for_multiple_samples(All_Samples_curr_3UTR_coverages, UTR_start, UTR_end, curr_strand, weight_for_second_coverage, Coverage_threshold, curr_3UTR_id):
    coverage_threshold = Coverage_threshold
    search_point_start = 150 ##200
    search_point_end = 100 #int(abs((UTR_end - UTR_start))*0.05)
    ...
    for i in range(num_samples):
        ...
        if max(curr_Region_Coverage_raw) < 10 :
            continue
        last_cov_idx = max(loc for loc, val in enumerate(curr_Region_Coverage_raw) if val >=10) #last value with cov>10


        if last_cov_idx <= search_point_start :
            continue

        dist_from_end = len(curr_Region_Coverage_raw) - last_cov_idx
        ...
        search_point_ends.append(dist_from_end)

    if len(search_point_ends)>0:
        last_cov = min(search_point_ends)

    if last_cov > search_point_end :
        search_point_end = last_cov
    ...
    if len(Pass_threshold_index) > least_pass_coverage_num and UTR_end - UTR_start >=150:
        ...
        for curr_point in range(search_region_start, search_region_end+1):
            ...
            for curr_sample_region_coverage in Region_Coverages:
                Mean_Squared_error, Long_UTR_abun, Short_UTR_abun = Estimation_abundance(curr_sample_region_coverage, curr_search_point, search_region_end, curr_3UTR_id)
        ...
        if len(Mean_squared_error_list) > 1:
            if max(Mean_squared_error_list) > 0:
                min_ele_index = Mean_squared_error_list.index(max(Mean_squared_error_list))

                select_mean_squared_error = Mean_squared_error_list[min_ele_index]
                selected_break_point = search_region[min_ele_index]

                UTR_abundances = [['NA']*num_samples, ['NA']*num_samples, 'NA']
                ...
                if num_samples == 2 and len(Pass_threshold_index) == 2:
                    stat, pvalue = scipy.stats.fisher_exact(UTR_abundances_passed)
                    UTR_abundances[2] = pvalue
...
def Estimation_abundance(Region_Coverage, break_point, search_region_end , curr_3UTR_id):

    Long_UTR_abun = np.mean(Region_Coverage[search_region_end-50:search_region_end])
    Short_UTR_abun = np.mean(Region_Coverage[break_point-50:break_point]) - np.mean(Region_Coverage[break_point:break_point+50])



    Mean_Squared_error = Short_UTR_abun**2
